...
public class RandomNumberGeneratorWorker extends Worker {
		...
		while (i<5 && !isStopped()){
			try{
				...
				if(mIsRandomGeneratorOn){
					...
					Log.i("WorkManagerDemo", "Thread id:" + Thread.currentThread().getId() + " " + getTags().toString());
					i++;
				}			
			}catch(InterruptedException e){
				Log.i("WorkManagerDemo", "Interrupted");
			}
		}
	}
	...
	public void onStopped() {
		super.onStopped();
		Log.i("WorkManagerDemo", "Worker has been stopped.");
	}
}